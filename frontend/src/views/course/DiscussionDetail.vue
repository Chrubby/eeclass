  ...
  <div class="max-w-4xl mx-auto p-4">
    <div class="bg-white p-6 rounded shadow-sm border mb-6">
      ...
      <div v-if="room.file_path" class="mt-5 pt-4 border-t border-gray-100">
        ...
        <button
          type="button"
          class="inline-flex items-center gap-1.5 text-sm font-bold text-[#337ab7] bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded transition-colors disabled:opacity-60"
        ...
        >
          ...
          {{ attachmentLoading ? '載入中...' : '檢視 PDF 檔案' }}
        </button>
      </div>
    </div>
    ...
    <div class="mb-8">
      <textarea
        v-model="newPost"
        class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none disabled:bg-gray-100"
        rows="3"
        placeholder="發表你的看法..."
    ...
    <div class="relative">
      <div class="space-y-6" :class="{ 'pointer-events-none opacity-60': isSubmitting }">
        <div
      ...
      <div
        v-if="isSubmitting"
        class="absolute inset-0 bg-white/60 flex items-center justify-center z-10 rounded-lg"
      >
        <div class="flex flex-col items-center gap-2 bg-white border shadow-sm rounded-lg px-5 py-4">
          <svg class="animate-spin h-6 w-6 text-[#337ab7]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
          </svg>
          <span class="text-sm text-gray-700 font-bold">發表中，請稍候...</span>
        </div>
      </div>
    </div>
  </div>
...
import { openFileInNewTab } from '@/utils/files.js'
...
const onViewAttachment = () => {
  if (!room.value.file_path) return
  attachmentLoading.value = true
  try {
    openFileInNewTab(room.value.file_path)
  } finally {
